...
@Component
public class CtripCheckStatementBiz extends CtripAbsRiverBiz {
    ...
    public CtripCheckStatementBiz() {
        super(Constant.CTRIP_CHECK_STATEMENT);
    }
    ...
    public String processLog(Map<String, Object> dataMap) {
        ...
        if (dataMap.containsKey("statementFile")) {
            builder.append("<b>Ctrip order check results</b>").append("\r\n\r\n");
            builder.append("Account statement date: ").append(dataMap.get("statementDate")).append("\r\n");
            builder.append("Account statement file: ").append(dataMap.get("statementFile")).append("\r\n");
            builder.append("Account statement order num: ").append(dataMap.get("orderNum")).append("\r\n");
            builder.append("No exist order num: ").append(dataMap.get("noExistOrderNum")).append("\r\n");
            builder.append("Repeat order num: ").append(dataMap.get("repeatOrderNum")).append("\r\n");
            builder.append("Large amt order num: ").append(dataMap.get("largeAmtOrderNum"));
        }
    ...
    public void processData(Map<String, Object> dataMap) {
        ...
        if(CollectionUtils.isNotEmpty(billFileList)){
            for (String billFilePath : billFileList) {
                try {
                    long statementId = this.saveOrderBillDetails(dataMap, billFilePath);
                    // 3、核对订单明细、按照三个维度：订单金额（对账单金额比本地多）、订单数量（对账单数量比本地多）、重复订单（对账单是否含有已经对过账的订单）
                    List<OrderCheckResultDetail> checkResultList = this.checkOrderStatement(dataMap,statementId);
    ...
    private long saveOrderBillDetails(Map<String, Object> dataMap, String billFilePath){
        // 保存对账主表信息
        ...
        dataMap.put("statementDate", statementDate);
...
//        CtripCheckStatementBiz csBiz = (CtripCheckStatementBiz)applicationContext.getBean("ctripCheckStatementBiz");
        ...
        System.out.println("lRefundRowModels.size---------------"+lRefundRowModels.size());
        refundNumTotal = lRefundRowModels.size();
        this.saveRefundStatements(statementId,paramMap, lRefundRowModels,cwIdRefMap);

        System.out.println("lOrderRowModels.size---------------"+lOrderRowModels.size());
        orderNumTotal = lOrderRowModels.size();
        this.saveOrderStatements(statementId,paramMap, lOrderRowModels,cwIdRefMap);
        System.out.println("totalMap.get(\"orderNumTotal\")---------------"+orderNumTotal);
        System.out.println("totalMap.get(\"refundNumTotal\")---------------"+refundNumTotal);
        ...
        dataMap.put("orderNum", orderNumTotal);
        dataMap.put("refundNum", refundNumTotal);
        dataMap.put("statementFile", billFilePath);
        return statementId;
    }
     ...
     * @param dataMap
     */
    private List<OrderCheckResultDetail> checkOrderStatement(Map<String, Object> dataMap,long statementId) {
        ...
        dataMap.put("noExistOrderNum", noExistOrderNum);
        ...
        dataMap.put("repeatOrderNum", repeatOrderNum);
        if (repeatOrderNum > 0) {
        ...
        dataMap.put("largeAmtOrderNum", largeAmtOrderNum);
    ...
    private void sendEmail(Map<String, Object> dataMap, List<OrderCheckResultDetail> checkResultList) {
        ...
        try {
            ...
            CommonSendMailUtils.sendMail(CommonSendMailUtils.MAIL_SENDER_CTRIP_CONST, subject,
                    orderCheckEmail, mailBody, orderCheckFile);
        } catch (Exception e) {
            e.printStackTrace();
